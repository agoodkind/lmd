...
| `LMD_BATTERY_THROTTLE_PCT` | int | 0..100 | `20` | Battery charge at or below which the GPU throttle engages. |
| `LMD_BATTERY_RESUME_PCT` | int | 0..100 | `80` | Battery charge at or above which the throttle releases (a wide band so it does not flap). |
