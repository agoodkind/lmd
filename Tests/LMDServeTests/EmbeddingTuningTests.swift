...
final class EmbeddingTuningTests: XCTestCase {
  ...
  func testTransientBytesPerSlotFormula() {
    ...
    expect(
      EmbeddingTuningResolver.transientBytesPerSlot(
  ...
  func testBudgetAutoFitsTwiceTransientsInCacheRoundedTo1024() {
    ...
    expect(
      EmbeddingTuningResolver.resolveSlotBudget(
        explicit: nil, cacheLimitBytes: Int(8 * self.gib), transientBytesPerSlot: 180_224))
      == 23_552
  }
  ...
  func testBudgetAutoClamps() {
    expect(
      EmbeddingTuningResolver.resolveSlotBudget(
        explicit: nil, cacheLimitBytes: Int(2 * self.gib), transientBytesPerSlot: 4_000_000))
      == 2_048
    expect(
      EmbeddingTuningResolver.resolveSlotBudget(
  ...
  func testExplicitBudgetWins() {
    expect(
      EmbeddingTuningResolver.resolveSlotBudget(
        explicit: 8_192, cacheLimitBytes: Int(2 * self.gib), transientBytesPerSlot: 180_224))
      == 8_192
  }
}
