...
final class MemoryMonitor {
  ...
  private func startMacmon() {
    ...
    do {
      ...
      log.notice(
        "macmon.started pid=\(p.processIdentifier, privacy: .public) port=\(self.macmonPort, privacy: .public)"
      )
    } catch {
...
func buildRepoDump(repo: String, maxBytes: Int) -> String {
  ...
  let skipDirs: Set<String> = [
    "node_modules", ".git", "dist", "build", "vendor", "target", "keepalived-src",
  ]
...
// Field names are the snake_case keys written to the bench result JSONL.
...
struct AnyCodable: Encodable {
  ...
  func encode(to encoder: Encoder) throws {
    ...
    default:
      if let n = value as? NSNumber { try c.encode(n.doubleValue) } else { try c.encodeNil() }
    }
  }
}
...
func runModel(displayName: String, path: String, maxTokens: Int) {
  ...
  do {
    ...
    log.error(
      "server.start_failed model=\(displayName, privacy: .public) error=\(String(describing: error), privacy: .public)"
    )
    return
  }
  ...
  _ = runChat(
    model: path, systemPrompt: "You are helpful.", userContent: "Reply with just: ok", maxTokens: 8)
  log.info(
    "server.warmup_done model=\(displayName, privacy: .public) elapsed_s=\(Int(Date().timeIntervalSince(warmupStart)), privacy: .public)"
  )

  log.notice(
    "server.ready model=\(displayName, privacy: .public) pending=\(pending.count, privacy: .public)"
  )
  ...
  for testType in pending {
    ...
    if userContent.isEmpty {
      log.info(
        "cell.skipped reason=empty_content model=\(displayName, privacy: .public) test=\(testType, privacy: .public)"
      )
      continue
    }
    log.notice(
      "cell.running model=\(displayName, privacy: .public) test=\(testType, privacy: .public) bytes=\(userContent.count, privacy: .public)"
    )
    let r = runChat(
      model: path, systemPrompt: sysPrompt, userContent: userContent, maxTokens: maxTokens)
    writeResult(r, to: outFile)
    log.notice(
      "cell.completed model=\(displayName, privacy: .public) test=\(testType, privacy: .public) elapsed_s=\(r.elapsed_seconds, privacy: .public) http=\(r.http_code, privacy: .public)"
    )
  }
...
func runReasoningModel(_ m: ReasoningModel) {
  ...
  do {
    ...
    log.error(
      "reasoning.server_start_failed model=\(m.displayName, privacy: .public) error=\(String(describing: error), privacy: .public)"
    )
    return
  }
  MemoryMonitor.shared.setCurrentServer(server)
  ...
  let warmupStart = Date()
  _ = runChat(
    model: m.path, systemPrompt: "You are helpful.", userContent: "Reply with just: ok",
    maxTokens: 16)
  log.info(
    "reasoning.warmup_done model=\(m.displayName, privacy: .public) elapsed_s=\(Int(Date().timeIntervalSince(warmupStart)), privacy: .public)"
  )
  ...
  for testType in pending {
    ...
    log.notice(
      "reasoning.cell_running model=\(m.displayName, privacy: .public) test=\(testType, privacy: .public) bytes=\(userContent.count, privacy: .public)"
    )
    let r = runChat(
      model: m.path, systemPrompt: sysPrompt, userContent: userContent, maxTokens: m.maxTokens)
    writeResult(r, to: outFile)
    log.notice(
      "reasoning.cell_completed model=\(m.displayName, privacy: .public) test=\(testType, privacy: .public) elapsed_s=\(r.elapsed_seconds, privacy: .public) http=\(r.http_code, privacy: .public)"
    )
  }
...
public enum LMDBenchTool {
  public static func run() {
    ...
    log.notice(
      "bench.starting models=\(models.count, privacy: .public) tests=\(tests.count, privacy: .public) total=\(models.count * tests.count, privacy: .public)"
    )
